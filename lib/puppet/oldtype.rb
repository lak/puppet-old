...
# All of the class functionality
# from the old Puppet::Type
class Puppet::OldType
  extend Puppet::Util # for symbolize()
  extend Puppet::Util::ClassGen # for genclass()

  # This module is used by Puppet::OldType at both the instance and class level.
  # This likely won't stay this way, but in the transition, it's easiest.
  module ParameterManagement
    ...
    def parameters
      ...
      # are defined in Puppet::OldType (rather than a subclass), then it's
    ...
    def parameter(name)
      ...
      if is_a?(Class)
        ...
        @parameters_by_name[name] || Puppet::OldType.parameter(name)
      end
    end
    ...
    # This method is used in both Puppet::OldType and Puppet::Resource.
    def valid_parameter?(name)
      name = name.to_sym
      return true if name == :name and ! is_a?(Class)
      return true if parameter(name)
      return false
    end
  end

  include ParameterManagement
  extend ParameterManagement
  ...
  def self.metaparameter?(param)
    Puppet::OldType.valid_parameter?(param)
  end

  def self.metaparameter(name)
    Puppet::OldType.parameter(name)
  end

  def self.metaparameters
    Puppet::OldType.parameters
  end
  ...
  # XXX This should be :boolean => true, right?
  ...
  def const_defined?(str)
    begin
      const = "Puppet::Type::#{name.to_s.capitalize}::#{str}"
      eval const
      puts "#{const}: yep"
      true
    rescue
